...
**Шифрование.** Название, сумма и категория каждой траты и лимиты хранятся зашифрованными
(AES-256-GCM, [cryptography](https://cryptography.io/)). Ключ данных каждого пользователя защищён его паролем:
в базе лежит только ключ, «обёрнутый» через scrypt(пароль) — сам пароль нигде не сохраняется. После ввода пароля
ключ живёт только в оперативной памяти (`/dev/shm/bot_raskhody`, очищается при перезагрузке) и удаляется после
30 дней без активности (`UNLOCK_TTL_DAYS`). Копия базы или диска без пароля пользователя нечитаема, в том числе
для владельца сервера. Забытый пароль восстановить нельзя: `/forgot` в боте или «Забыли пароль?» в приложении
удаляют данные пользователя. Онлайн-перебор ограничен 5 попытками за 15 минут, офлайн-перебор дорог из-за scrypt.
В боте пароль вводится сообщением, которое бот сразу удаляет; в приложении — напрямую по HTTPS, минуя Telegram.
Действие `reset-data` в workflow удаляет все данные.
