...
**Шторки.** Лист снизу закрывается свайпом вниз (тянем от верха содержимого), нажатием вне листа или
кнопкой «Назад» Telegram.

**Справка.** Кнопка «❓ Что умею» на клавиатуре бота и команда `/help` присылают подробную инструкцию
(`HELP_TEXT` в `main.py`); список команд бота задаётся в `post_init` через `set_my_commands`. В приложении
та же справка живёт в ⚙️ → «Как пользоваться».

**Настройки и валюта.** Иконка шестерёнки в приложении: основная валюта (по умолчанию RUB; полный список
ISO 4217 с поиском и избранным, `currencies.py`), лимиты, админка для владельца. Каждая трата хранит сумму и валюту ввода; показывается
в основной валюте пользователя по текущему курсу (`rates.py`: open.er-api.com, запасной источник ЦБ РФ, кэш в
базе, обновление раз в 6 часов). При смене основной валюты лимиты пересчитываются по курсу. В форме траты
можно выбрать валюту из избранных; чеки по QR всегда в рублях.
